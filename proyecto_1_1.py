...
Original file is located at
    https://colab.research.google.com/drive/10NV_8XjzINNUp-fQvXYWXbQJpjV0dl9N
"""
...
def construir_red(df):
    ...
    for i, fila in df.iterrows():  # Itera sobre las filas del DataFrame.
        ...
        descripcion = fila['Descripción']  # Obtiene la descripción de la actividad.
        precedentes = fila['Precedentes']  # Obtiene los precedentes de la actividad.
        duracion = fila['Duración']  # Obtiene la duración de la actividad.

        if pd.isna(precedentes) or precedentes == '-':  # Comprueba si no hay precedentes.
            nodo_inicial = actividad  # Asigna la actividad como nodo inicial.
            precedentes = []  # Establece una lista vacía de precedentes.
        else:
            precedentes = [int(p) for p in str(precedentes).split(',') if p]  # Convierte los precedentes a una lista de enteros.
        ...
        for p in precedentes:  # Itera sobre los precedentes.
            if p in actividades:  # Comprueba si el precedente existe en las actividades.
                ...
                print(f"Advertencia: La actividad {p} no existe y se omitirá como precedente de {actividad}.")  # Advierte si un precedente no existe.

    if len(nodos_sin_sucesores) != 1:  # Comprueba si hay exactamente un nodo final sin sucesores.
        raise ValueError("Debe haber exactamente un nodo final sin sucesores.")  # Lanza un error si no se cumple la condición.
...
def main():  # Define la función principal del programa.
    ...
    generar_reporte(actividades, nodo_inicial, nodo_final, fechas_mas_proximas, fechas_mas_lejanas)  # Genera un reporte del proyecto.
